...
var Settings={isMobile:Events.browser.isMobile,submenuLight:$header.hasClass('submenu-light')==!0?!0:!1,menuIsOpen:!1,menuOverlayOpened:!1}
...
searchBtn.on('click',function(){openSearch();return!1});searchBtnClose.on('click',function(){closeSearch();return!1});document.addEventListener('keyup',function(ev){if(ev.keyCode==27){closeSearch()}})}},mainMenu:function(){if($mainMenu.length>0){var $menuItemLinks=$("#mainMenu nav > ul > li.dropdown > a, .dropdown-submenu > a, .dropdown-submenu > span, .page-menu nav > ul > li.dropdown > a"),$triggerButton=$("#mainMenu-trigger a, #mainMenu-trigger button"),darkClassRemoved,processing=!1,triggerEvent;$triggerButton.on('click',function(e){var elem=$(this);e.preventDefault();$(window).breakpoints("lessThan","lg",function(){var openMenu=function(){if(!processing){processing=!0;Settings.menuIsOpen=!0;if(Settings.submenuLight){$header.removeClass('dark');darkClassRemoved=!0}
elem.addClass('toggle-active');$body.addClass('mainMenu-open');INSPIRO.header.logoStatus();$mainMenu.animate({'min-height':$window.height()},{duration:500,easing:"easeInOutQuart",start:function(){setTimeout(function(){$mainMenu.addClass('menu-animate')},300)},complete:function(){processing=!1}})}};var closeMenu=function(){if(!processing){processing=!0;Settings.menuIsOpen=!1;INSPIRO.header.logoStatus();$mainMenu.animate({'min-height':0},{start:function(){$mainMenu.removeClass('menu-animate')},done:function(){$body.removeClass('mainMenu-open');elem.removeClass('toggle-active');if(Settings.submenuLight&&darkClassRemoved){$header.addClass('dark')}},duration:500,easing:"easeInOutQuart",complete:function(){processing=!1}})}};if(!Settings.menuIsOpen){triggerEvent=openMenu()}else{triggerEvent=closeMenu()}})});$menuItemLinks.on('click',function(e){$(this).parent('li').siblings().removeClass('hover-active');$(this).parent('li').toggleClass('hover-active');e.stopPropagation();e.preventDefault()});$body.on('click',function(e){$mainMenu.find('.hover-active').removeClass('hover-active')});$(window).breakpoints("greaterEqualTo","lg",function(){var $menuLastItem=$("nav > ul > li:last-child"),$menuLastItemUl=$("nav > ul > li:last-child > ul"),$menuLastInvert=$menuLastItemUl.width()-$menuLastItem.width(),$menuItems=$("nav > ul > li").find(".dropdown-menu");$menuItems.css('display','block');$('.dropdown:not(.mega-menu-item) ul ul').each(function(index,element){if($window.width()-($(element).width()+$(element).offset().left)<0){$(element).addClass('menu-invert')}});if($window.width()-($menuLastItemUl.width()+$menuLastItem.offset().left)<0){$menuLastItemUl.addClass('menu-last')}
...
$inspiroSlider.each(function(){var elem=$(this);elem.options={cellSelector:elem.attr('data-item')||!1,prevNextButtons:elem.data('arrows')==!1?!1:!0,pageDots:elem.data('dots')==!0?!0:!0,fade:elem.data('fade')==!0?!0:!1,draggable:elem.data('drag')==!0?!0:!1,freeScroll:elem.data('free-scroll')==!0?!0:!1,wrapAround:elem.data('loop')==!1?!1:!0,groupCells:elem.data('group-cells')==!0?!0:!1,autoPlay:elem.attr('data-autoplay')||8000,pauseAutoPlayOnHover:elem.data('hoverpause')==!1?!1:!0,adaptiveHeight:elem.data('adaptive-height')==!1?!1:!1,asNavFor:elem.attr('data-navigation')||!1,selectedAttraction:elem.attr('data-attraction')||.07,friction:elem.attr('data-friction')||.9,initialIndex:elem.attr('data-initial-index')||0,accessibility:elem.data('accessibility')==!0?!0:!1,setGallerySize:elem.data('gallery-size')==!1?!1:!1,resize:elem.data('resize')==!1?!1:!1,cellAlign:elem.attr('data-align')||'left',};elem.find('.slide').each(function(){if($(this).hasClass("kenburns")){var elemChild=$(this),elemChildImage=elemChild.css('background-image').replace(/.*\s?url\([\'\"]?/,'').replace(/[\'\"]?\).*/,'');if(elemChild.attr('data-bg-image')){elemChildImage=elemChild.attr('data-bg-image')}
elemChild.prepend('<div class="kenburns-bg" style="background-image:url('+elemChildImage+')"></div>')}});elem.find(".slide video").each(function(){this.pause()});if(elem.find('.slide').length<=1){elem.options.prevNextButtons=!1;elem.options.pageDots=!1}
...
return!0}
var elem=$(".carousel");elem.flickity({adaptiveHeight:!1,wrapAround:!0,cellAlign:'left',resize:!1,on:{ready:function(){elem.addClass("carousel-loaded")},}})},carousel:function(elem){if(elem){$carousel=elem}
...
if(elem.options.direction=="forward"){new Waypoint({element:elem,handler:function(){var t=setTimeout(function(){elem.addClass(elem.options.animation+" visible")},elem.options.delay);this.destroy()},offset:'100%'})}else{elem.addClass("visible");elem.on("click",function(){elem.addClass(elem.options.animation);return!1})}
...
elem.gmap3({center:[Number(elem.options.latitude),Number(elem.options.longitude)],zoom:Number(elem.options.zoom),mapTypeId:google.maps.MapTypeId[elem.options.maptype],scrollwheel:!1,zoomControl:!0,mapTypeControl:!1,streetViewControl:!0,fullscreenControl:!0,styles:mapsStyle})
...
return!0}
$popover.popover({container:'body',html:!0})}},magnificPopup:function(){var $lightbox=$('[data-lightbox]');if($lightbox.length>0){if(typeof $.fn.magnificPopup==='undefined'){INSPIRO.elements.notification("Warning","jQuery magnificPopup plugin is missing in plugins.js file.","danger")
return!0}
...
if($btnModal.length>0){$btnModal.each(function(){var elem=$(this),modalTarget=elem.attr("data-modal");elem.click(function(){$(modalTarget).toggleClass(modalShowClass,1000);return!1})})}},notification:function($title,$message,$type,$element,$delay,$placement,$animateEnter,$animateExit){var $element,$elementContainer,$animateEnter=$animateEnter||"fadeInDown",$animateExit=$animateExit||"fadeOutDown",$placement;if($placement){$placement=$placement}else{$placement="top"}
if($element){$elementContainer="element-container";$animateEnter="fadeIn",$animateExit="fadeOut"}else{$elementContainer="col-11 col-lg-3"}
if(!$message){$message=""}
$element='body';$.notify({title:$title,message:$message},{element:$element,type:$type||'warning',delay:$delay||10000,template:'<div data-notify="container" class="bootstrap-notify '+$elementContainer+' alert alert-{0}" role="alert">'+'<button type="button" aria-hidden="true" class="close" data-notify="dismiss">Ã—</button>'+'<span data-notify="icon"></span> '+'<span data-notify="title">{1}</span> '+'<span data-notify="message">{2}</span>'+'</div>',mouse_over:!0,allow_dismiss:!0,placement:{from:$placement},animate:{enter:'animated '+$animateEnter,exit:'animated '+$animateExit}})},sidebarFixed:function(){if(INSPIRO.core.rtlStatus()){return!0}
