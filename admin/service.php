...
if(isset($_POST['submit'])){
  ...
  $service_des = validate($_POST['service_des']);
...
$sql =mysqli_query($con,"INSERT INTO `service`(`catid`, `name`, `sdetail`, `cost`, `cashback`,`advrequired`, `advper`, `duedays`, `duepay`, `type`, `pic`, `serdes`) VALUES ('".$categorya."','".$namea."', '".$servicedetail."' ,'".$servicecosta."','".$cashback."','".$advancea."','".$advancepercentagea."','".$serviceduedaysa."','".$paymentduedaysa."','".$servicetypea."','".$photo."', '".$service_des."')")or die(mysqli_error($con));
...
//echo 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';echo $fqnw;
   ...
   foreach ($_POST['multi_question'] as $key => $value) {
    ...
    //echo $value;
$certi = mysqli_query($con,'UPDATE `form_question` SET `question`="'.$value.'" WHERE service_id = "'.trim($_GET['edit']).'" AND q_list_id="'.$fqdata[$qid]['q_list_id'].'"') or die(mysqli_error($con));
...
$d_less =  mysqli_query($con, ' DELETE FROM  form_question WHERE service_id = "'.trim($_GET['edit']).'" AND q_list_id="'.$fqdata[$i]['q_list_id'].'" ');
 ...
 echo 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';echo count($fqdata)-count($_POST['multi_question']);
  for($j=0; $j<count($_POST['multi_question']); $j++){
    ...
    $certi = mysqli_query($con,'UPDATE `form_question` SET `question`="'.$_POST['multi_question'][$j].'" WHERE service_id = "'.trim($_GET['edit']).'" AND q_list_id="'.$fqdata[$i]['q_list_id'].'"') or die(mysqli_error($con));
  }
...
for ($i=0; $i <count($_POST['multi_question']) ; $i++) {
  ...
  if($i<$to_up){
    ...
    $certi = mysqli_query($con,'UPDATE `form_question` SET `question`="'.$_POST['multi_question'][$i].'" WHERE service_id = "'.trim($_GET['edit']).'" AND q_list_id="'.$fqdata[$i]['q_list_id'].'"') or die(mysqli_error($con));
  }else{
//$i--;
...
<div class="content-wrapper">
  ...
  <label class="mr-3 mb-0">Service Detail</label>
  <textarea class="form-control" name="servicedetail" rows="4" cols="120" placeholder="About the service detail will be here"><?php if(!empty($runservicef['sdetail'])){echo $runservicef['sdetail'];} ?></textarea>
</div>
...
<?php }?></select></div>
  ...
  <textarea class="form-control ck" id="service_des" name="service_des" rows="4" cols="120" placeholder="Enter service description"></textarea>
  <script>
...
<script>
    ClassicEditor
        .create( document.querySelector( '#service_des' ) )
        .catch( error => {
            console.error( error );
        } );
</script>
    ...
    $("#rowAdder").click(function () {
      ...
      `<div class="col-md-9">` +
        `<div class="input-group">` +
          `<input type="text" class="form-control m-input border border-dark" name="multi_question[]" style="width: 1160px; border: 1px solid #555;">` +
        `</div>` +
      `</div>` +
      `<div class="col-md-3 text-right" style="padding-right:3%;">` +
        `<button class="btn btn-danger" id="DeleteRow" type="button">` +
          `<i class="fa fa-remove"></i>` +
        `</button>` +
      `</div>` +
...
$("#rowwAdder").click(function () {
  newRowAdd = 
    `<div id="row" class="row m-3">` +
      `<div class="col-md-9">` +
        `<div class="input-group">` +
          `<input type="text" class="form-control m-input border border-dark" name="multi_question[]" placeholder="Enter Question Here..." style="width: 1160px; border: 1px solid #555;">` +
        `</div>` +
      `</div>` +
      `<div class="col-md-3 text-right" style="padding-right:3%;">` +
        `<button class="btn btn-danger" id="DeleteRow" type="button">` +
