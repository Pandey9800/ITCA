...
<div class="col-md-4"></div><div class="col-md-4">
          ...
          <!-- /.box -->
            ...
            <?php }?>
              <h3 class="box-title" style="color:#000;">Form List</h3></div>
            <div class="box-body">
            <div class="row" style="margin-top: 20px; margin-left: 5px;margin-right: 5px">
          <table class="table table-striped" id="example1"><thead><tr><th>Sno.</th><th>Name</th><th>Assign To</th><th>Status</th><th>Apply On</th><th>View</th></tr></thead><tbody><tr>
<?php $i=1;
$ress=mysqli_query($con,"SELECT `id`,`assign`, `stfid`, `coupan`, `taskid`, `taskamt`, `taskdiscamt`, `status`, `timedate`, `ip` FROM `task` WHERE `assign`!=0 ORDER BY `timedate` DESC");
while($rows=mysqli_fetch_array($ress)){?>
<td><?php echo $i; ?>.</td><th><?php echo ServiceName($rows['taskid']); ?></th><td><?php echo EcounterName($rows['assign']); ?></td><td><?php echo applicationStatus($rows['status']); ?></td><td><?php echo showdate($rows['timedate']); ?></td>
<td><a href="all-form?astaskid=<?php echo ($rows['id']); ?>" class="btn btn-primary btn-sm">Re-Assign</a></td>
</tr><?php  $i++;}?></tbody>
</table>
</div></div>
</div>
