  ...
  <body class="skin-blue">
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
       ...
       <!-- /.content -->
<!-- /.row -->
<a href="acc_indx_coun_page.php">< BACK</a>
...
ALL DETAILS 
 ...
 <?php
  ...
  $des=$_POST["countName"];
   $ress=mysqli_query($con,"SELECT c.* , p.* FROM transactions c,ecounter p WHERE c.transfer_from = p.eno AND transfer_from='$des'");
				  $res=mysqli_query($con,"SELECT c.* , p.first_name FROM transactions c,ecounter p WHERE c.transfer_to = p.eno AND transfer_from='$des' ");
...
<?php
$sql=mysqli_query($con,"SELECT sum(amount) AS sum FROM 
  transactions WHERE `date_time` BETWEEN DATE_SUB( CURDATE( ) ,INTERVAL 1 MONTH ) AND DATE_SUB( CURDATE( ) ,INTERVAL 0 MONTH) AND  transfer_from='$des'");
  while ( $row = mysqli_fetch_array ( $sql, MYSQLI_ASSOC ) )
	{
echo'Current Month Total Transaction-'; echo $row['sum'];
        }
