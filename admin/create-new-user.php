...
$sql =mysqli_query($con,"UPDATE `ecounter` SET `username`='".trim($_GET['mob'])."',`status`=2 WHERE `eno`='".trim($_GET['eid'])."'");
if($sql>0){
...
$email_from = 'uchiha.nanami98@gmail.com';
$email_subject = "Yo! $first_name";
$email_body = "You have received a new camp registration from the user $first_name.\r\n" .
              "Name: $first_name\r\n" .
              // "Email: $emailida\r\n" .
              // "Phone: $contacta\r\n";
$headers = "From: $email_from\r\n";
$headers .= "Reply-To: $emailida\r\n";
            ...
            <div class=col-sm-5>
                ...
                <div class=col-sm-4>
                    <div class="form-group mb-10">
                        ...
                        <input type="text" pattern="[0-9]+" maxlength="10" class="form-control" name="contact" placeholder="Enter Here" required="required" />
                    </div>
                </div>
                <div class=col-sm-4>
                    <div class="form-group mb-10">
                        <label>WhatsApp Number :<span style="color:red"></span></label>
                        <input type="text" pattern="[0-9]+" maxlength="10" class="form-control" name="wacontact" placeholder="Enter Here" />
                    </div>
                </div>
                <div class=col-sm-4>
                    <div class="form-group mb-10">
...
<option value="Surguja">Surguja</option>
       ...
       <option value="freelance">E-counter</option>
       <option value="refer">Partner/Refer</option>
                        </select>
                    </div>
                </div>
               <div class=col-sm-4>
      ...
      <?php }else{ ?>
          ...
          <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
              <li class="active"><a href="#tab_1" data-toggle="tab">E-Counter </a></li>
              <li><a href="#tab_2" data-toggle="tab">Partner/ Referel</a></li>
              <li><a href="#tab_3" data-toggle="tab">Employees</a></li>
            </ul>
               ...
               <table class="table table-striped" class="example1">
                <thead>
                  <tr><th>Sno.</th><th>Name</th>
                    <th>Login Details</th>
                    <th>Contact Details</th>
                    <th>City</th><th>Add On</th><th>Status</th></tr>
                </thead>
                <tbody>
                  <tr>
                 <?php $i=1;
          $ress=mysqli_query($con,"SELECT `eno`, `username`, `memcode`, `first_name`, `present_address`, `present_city`, `present_district`, `present_state`, `present_pin_code`, `present_country`, `sex`, `dob`, `pan_number`, `mobile_no`, `tel_phone_no`, `qualificaton`, `other_qualification`, `Email`, `Password`, `pan_wallet`, `e_wallet`, `reward_wallet`, `ecounter`,`status`,`timedate`, `ip` FROM `ecounter` WHERE `employeetype`='refer' ORDER BY `timedate` DESC");
           while($rows=mysqli_fetch_array($ress)){?>
<td><?php echo $i; ?>.</td><td><?php echo $rows['first_name']; ?></td><td><?php echo $rows['username'].'<br/>'.$rows['Password']; ?></td><td><?php echo $rows['mobile_no'].'<br/>'.$rows['tel_phone_no'].'<br/>'.$rows['Email']; ?></td><td><?php echo $rows['present_city']; ?></td><td><?php echo showdate($rows['timedate']); ?></td><td><?php if($rows['status']!=1){ ?><i class="fa fa-check-circle fa-2x text-green"><?php }else{ ?><a href="create-new-user?eid=<?php echo $rows['eno']; ?>&mob=<?php echo $rows['mobile_no']; ?>" class="btn btn-primary btn-sm">Make Active</a><?php }?></td>
                  </tr>
                  <?php  $i++;}   ?>
