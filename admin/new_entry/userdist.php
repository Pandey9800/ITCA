<?php
 ...
 include_once 'header.php'; ?>
        ...
        <div class="content-wrapper">
            ...
            <div align="center"><label><font color="#000099">Select Work Area</font></label></div>
	...
	<?php
                    ...
                    <div class="form-group mb-10">
                        ...
                        <?php
     //  $queryusers = "SELECT `eno`,`zone` FROM `ecounter` WHERE `zone` IS NOT NULL";
//       $db = mysqli_query($con,$queryusers);
//       while ( $d=mysqli_fetch_assoc($db)) {
//	   $rid=$d['zone'];
//	   $rstate=$d["zone"];?>
	...
	<?php
	} ?> </select><button type="button" class="" data-toggle="modal" data-target="#myModal">Add New</button>
					  </div>
                    </div>
			
           <!--  <div class=col-sm-4>
                    <div class="form-group mb-10">
                        <label>Select District:<span style="color:red"></span></label><br />
                        AA:<input type="radio" value="21"  class="" name="dist"  /><br/>
                        BB:<input type="radio" value="12"  class="" name="dist"  /><br/>
                        CC:<input type="radio"  value="11" class="" name="dist"  />
                       </div>
                    </div>-->
                      <div class=col-sm-4>
                     <div class="form-group mb-10">
                        <label>Farm Name<span style="color:red">*</span></label>
                        <input type="text" class="form-control" name="farm" required />
                    </div>
                </div>
            </div>
              <div align="center"><label><font color="#990000">Enter New Member District Details</font></label></div>
            <div class="row" style="margin-top: 20px; margin-left: 5px;margin-right: 5px">
                ...
                <div class=col-sm-4>
                 ...
                 <div class=col-sm-4>
                    ...
                    <div class="form-group mb-10">
                        ...
                        <input type="text" value="<?php echo $_GET['name']; ?>" class="form-control" name="rollname" required />
                      </div>
                    </div> 
                     </div>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="ajaxfordist.php" method="post">
          <?php $sf=mysqli_query($con,"SELECT `id`, `state`, `dist`, `tehsil` FROM `level` WHERE `state` IS NOT NULL GROUP BY state"); ?>
          <select class="form-control" name="ss1" required >
          <option >Select State</option>
		<?php  while($sl=mysqli_fetch_array($sf)){?>
        
         <option value="<?php echo $sl['id'] ?>"><?php echo $sl['state'] ?></option>
         <?php } ?>
          </select>
          <input type="text" placeholder="Enter District Name" class="form-control" name="ss" />
...
//   $c_ip = '1';
//   
   $sql = "INSERT INTO `ecounter`(`prev`,memcode, `first_name`, `middle_name`, `last_name`, `father_name`, `present_address`, `present_city`, `present_district`, `present_state`, `present_pin_code`, `present_country`, `perm_address`, `perm_city`, `perm_district`, `perm_state`, `perm_pin_code`, `perm_country`, `sex`, `dob`, `martial_status`, `pan_number`, `mobile_no`, `tel_phone_no`, `qualificaton`, `other_qualification`,`Email`, `Password`, `pan_wallet`, `e_wallet`, `reward_wallet`,`state`,`zone`,`dist`,`fname`,ip)VALUES('$roll','$fcode','$first_name','$middle_name','$last_name','$father_name','$present_address','$present_city','$present_district','$present_state','$present_pin_code','$present_country','$perm_address','$perm_city','$perm_district','$perm_state','$perm_pin_code','$perm_country','$sex','$dob','$martial_status','$pan_number','$mobile_no','$tel_phone_no','$qualification','$other_qualification','$Email','$pass',0,0,0,'$state','$zone','$dist','$farm','".$_SERVER['REMOTE_ADDR']."')";
   if (mysqli_query($con, $sql)) {
    echo "<script>alert('District! Account Created Successfully, Thank You!')</script>";
		echo "<script>window.open('userdist.php?name=District','_self')</script>";
} else {
