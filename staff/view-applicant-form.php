...
if(isset($_GET['message'])) {
    // Sanitize the task ID parameter to prevent SQL injection
    $taskId = mysqli_real_escape_string($con, $_GET['id']);

    // Query the task data from the database
    $tinfoq = mysqli_query($con, "SELECT * FROM `task` WHERE id='$taskId'");
    $tdata = mysqli_fetch_assoc($tinfoq);

    // Check if the task data is retrieved successfully
    if ($tdata) {
        // Extract the employee staff ID and worker staff ID from the task data
        $estfid = $tdata['stfid'];
        $wstfid = $tdata['assign'];

        // If assign is 0, set wstfid to 0
        if ($wstfid == 0) {
            $wstfid = $estfid; // Assign the employee staff ID as worker staff ID
            $estfid = $tdata['assign']; // Assign the worker staff ID as employee staff ID
        }

        // Sanitize the message parameter to prevent SQL injection
        $message = mysqli_real_escape_string($con, $_GET['message']);

        // Insert the message into the recent chat table
        $sendq = mysqli_query($con, "INSERT INTO `recentchat`(`estfid`, `wstfid`, `echat`, `taskid`, `taskchatid`) 
                                     VALUES ('$estfid', '$wstfid', '$message', '$taskId', '{$tdata['id']}')");

        // Check if the query was executed successfully
        if ($sendq) {
            echo "<script>window.open('view-applicant-form?id=$taskId', '_self')</script>";
        } else {
