...
if(isset($_SESSION['memid'])){ header("Location:dashboard"); }
if(isset($_POST['login_admin'])){
    $name = mysqli_real_escape_string($con,$_POST['name']);
    $apass = mysqli_real_escape_string($con,$_POST['password']);
    $res=mysqli_query($con,"SELECT `eno`, `password` FROM ecounter WHERE `username`='$name'");
    if(mysqli_num_rows($res)>0){
    $row=mysqli_fetch_array($res);
    if($row['password']==$apass){
        $_SESSION['memid'] = $row['eno'];
...
<html>
    <head>
        ...
        <title>Staff Managment Panel</title>
        <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport"><link href="//maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" rel="stylesheet" id="bootstrap-css">
    <script src="//maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js"></script><script src="//code.jquery.com/jquery-1.11.1.min.js">
    </script>
